    ...
    <AnimatePresence>
      {visible && (
        <motion.div
          ...
          exit={{ opacity: 0 }}
          className={cn(
            "absolute inset-0 z-50 flex flex-col items-center justify-center bg-[#060d18]/85 backdrop-blur-md",
            className,
          )}
        >
          ...
          <p className="mt-5 text-sm text-slate-300 tracking-wide">{message}</p>
        </motion.div>
      )}
    </AnimatePresence>
  );
